...
class AccountCheckDeposit(models.Model):
    ...
    def _compute_check_deposit(self):
        rg_res = self.env["account.move.line"]._read_group(
            ...
            ["check_deposit_id", "amount_currency:sum", "debit:sum", "id:count"],
            ["check_deposit_id"],
        )
        mapped_data = {
            group["check_deposit_id"][0]: {
                "debit": group.get("debit", 0.0),
                "amount_currency": group.get("amount_currency", 0.0),
                "count": group.get("id_count", 0),
            }
            for group in rg_res
        }
        for deposit in self:
            if deposit.company_id.currency_id != deposit.currency_id:
                total = mapped_data.get(deposit.id, {"amount_currency": 0.0})["amount_currency"]
            else:
